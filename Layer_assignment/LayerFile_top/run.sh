...
if [ "$3" == "" ]; then
	echo "Provide target pd percent on top layer"
	echo "0~1"
	exit
fi
...
echo "./LA -format $Mode -bench $BenchPath -nparts $Layer -rseed $seed  -output 1 -OutputFile $OutputFile"
time ./LA -format $Mode -bench $BenchPath -nparts $Layer -rseed $seed  -output 1 -OutputFile $OutputFile -toptarget $pdTarget
